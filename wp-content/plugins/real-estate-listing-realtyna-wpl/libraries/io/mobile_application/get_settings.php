...
class wpl_io_cmd_get_settings extends wpl_io_cmd_base
{
    ...
    private function create_wizard_fragment()
    {
        ...
        $fields = wpl_flex::get_fields('', 0, 0, '', '', "AND `enabled` >= '1' AND `addon_mobile_wizard` >= '1' AND `kind` = '0' AND `category` IN ({$category_ids}) AND `type` IN ({$field_types})");
        ...
        foreach ($fields as $field)
        {
            ...
            elseif($field->type == 'locations')
            {
                ...
                for($i = 1; $i <= 7; $i++)
                {
                    ...
                    $locations = $i == 1 ? wpl_locations::get_locations($i, '', 1) : array();
