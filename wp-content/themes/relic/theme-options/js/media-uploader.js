(function($) {
    $(document).ready(function() {
        function relic_add_file(event, selector) {
            ...
            frame.on('select', function() {
                ...
                if (attachment.attributes.type == 'image') {
                    selector.find('.screenshot').empty().hide().append('<img src="' + attachment.attributes.url + '"><a class="remove-image"></a>').slideDown('fast');
					 ...
					 selector.find('.error-msg').removeClass('hide');
					 selector.find('.error-msg').text('Please upload image only!!');
					 selector.find('.upload').val("");
					 selector.find('.remove-image, .remove-file').trigger("click");
					return false;
				}
				selector.find('.error-msg').addClass('hide');
				selector.find('.error-msg').text('');
                ...
                selector.find('.remove-image, .remove-file').on('click', function() {
                    relic_remove_file($(this).parents('.section'));
                });
            });
            // Finally, open the modal.
            frame.open();
        }
        function relic_remove_file(selector) {
            ...
            selector.find('.upload-button').live('click', function() {
                relic_add_file(event, $(this).parents('.section'));
            });
        }
